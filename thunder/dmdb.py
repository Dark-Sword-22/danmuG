...
class DAL:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if cls._global_lock is None:
            cls._global_lock = asyncio.Lock(loop = kwargs["loop"])
        instance.global_lock = cls._global_lock
        return instance

    def __init__(self, db_engine, db_session: Session, msg_core, logger = None, loop = None):
    ...
    async def client_confirm_quest(self, bvid: str, qid: int, token: str):
        ...
        if item and item.status == 0:
            if compare_digest(self.create_token(f"{item.send_time}-{item.rnd}"), token):
                item.status = 1
                await self.session.commit()
                self.loop.create_task(self.task_status_daemon(table, qid))
                return True
            else:
                return -3
        else:
